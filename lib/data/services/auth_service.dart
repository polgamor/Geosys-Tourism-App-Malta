...
class AuthService {
  ...
  // --- Session Management (sin cambios en la lógica interna) ---
  ...
  Future<Session?> recoverStoredSession() async {
    ...
    try {
      ...
      // Si la sesión almacenada es inválida, la limpiamos
      await _clearSession();
      return null;
    }
  }
  ...
  Future<bool> signUpWithEmail({
    ...
    try {
      ...
      if (e is EmailInUseAuthException) {
        rethrow;
      }
  ...
  /// (Funcionalidad interna sin cambios, pero ahora lanza excepciones en lugar de mostrar SnackBars)
  ...
  /// (Funcionalidad interna sin cambios, pero ahora lanza excepciones)
  ...
  Future<void> verifyPasswordResetOtp({
    ...
    try {
      ...
      if (response.session != null) {
        ...
        // Si la verificación es exitosa pero no devuelve una sesión,
        // el usuario puede proceder a la pantalla de nueva contraseña.
        // Supabase a menudo no devuelve una sesión aquí, pero valida el token,
        // lo que permite llamar a `updateUser` en el siguiente paso.
        // Por tanto, no lanzamos una excepción si la sesión es nula.
      }
    } on AuthException catch (e) {
  ...
  Future<void> signOut() async {
    ...
    // FIX: Maneja el error si no hay sesión de Facebook
    try {
      await FacebookAuth.instance.logOut();
    } catch (_) {
      // Ignora el error si el usuario no estaba logueado con Facebook
    }
  }
  ...
  /// Comprueba si el usuario actual ha completado el proceso de onboarding.
  // REEMPLAZA EL MÉTODO ANTIGUO CON ESTE
  Future<bool> isCurrentUserOnboardingComplete() async {
    final userId = supabase.auth.currentUser?.id;
    if (userId == null) {
      // Si no hay usuario, no se puede verificar. Asumimos que no necesita onboarding.
      return true;
    }
    ...
    try {
      ...
      // Si falla (probablemente porque el perfil aún no existe), esperamos 2 segundos.
      debugPrint('Perfil no encontrado en el primer intento, reintentando... Error: $e');
      ...
      try {
        ...
        // Si vuelve a fallar, es seguro asumir que el onboarding es necesario.
        debugPrint('Error en el segundo intento al verificar onboarding: $finalError');
        return false;
      }
    }
  }

  /// Actualiza el perfil del usuario con los datos del onboarding y lo marca como completo.
  Future<void> completeOnboarding(Map<String, dynamic> profileData) async {
    ...
    final dataToUpdate = {
      ...profileData,
      'onboarding_complete': true, // ¡La parte más importante!
      'updated_at': DateTime.now().toIso8601String(),
    };

    // Filtra cualquier valor nulo o vacío que el usuario haya omitido
    dataToUpdate.removeWhere((key, value) => value == null || (value is String && value.isEmpty));

    try {
      await supabase.from('profiles').update(dataToUpdate).eq('id', userId);
    } catch (e) {
      debugPrint('Error completing onboarding: $e');
      // Re-lanza una excepción más específica si es necesario
      throw GenericAuthException();
    }
  }
  ...
  /// Deletes the current user's account after confirming their password.
  Future<void> deleteAccount(String password, String reason) async {
    final user = supabase.auth.currentUser;
    if (user == null || user.email == null) throw GenericAuthException();

    try {
      // 1. Re-authenticate to verify password before deletion
